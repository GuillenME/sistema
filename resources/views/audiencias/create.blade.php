    ...
    <form action="{{ route('audiencias.store') }}" method="POST" class="form-grid">
            ...
            <select id="delitos_id" name="delitos_id" required>
                ...
                @foreach($delitos as $delito)
                    <option value="{{ $delito->id }}"
                        {{ old('delitos_id') == $delito->id ? 'selected' : '' }}>
                        {{ $delito->delito }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group full" data-imputados-picker>
            ...
            <div class="imputados-panel">
                ...
                <input
                    type="text"
                    data-imputados-search
                    placeholder="Buscar imputado por nombre o apellidos"
                    autocomplete="off">
                ...
                <div class="imputados-list" data-imputados-list>
                    @forelse($imputados as $imputado)
                        ...
                        <label class="imputado-option" data-imputado-option data-search="{{ Str::lower($fullName) }}">
                            <input
                                type="checkbox"
                                name="imputados[]"
                                value="{{ $imputado->id }}"
    ...
    document.querySelectorAll('[data-imputados-picker]').forEach(function (picker) {
        ...
        function updateHelp() {
            ...
            if (currentMode() === 'single') {
                help.textContent = selectedCount ? 'Hay 1 imputado seleccionado.' : 'Selecciona el imputado de la lista.';
                return;
            }

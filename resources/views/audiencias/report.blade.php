    ...
    <div class="page-shell">
        ...
        <div class="panel" style="padding: 24px;">
            <form method="GET" action="{{ route('audiencias.report') }}" class="form-grid">
                <div class="form-group">
                    ...
                    <input type="date" id="fecha_desde" name="fecha_desde" value="{{ old('fecha_desde', $filters['fecha_desde'] ?? '') }}">
                    @error('fecha_desde') <div class="form-error">{{ $message }}</div> @enderror
                </div>

                <div class="form-group">
                    <label for="fecha_hasta">Fecha hasta</label>
                    <input type="date" id="fecha_hasta" name="fecha_hasta" value="{{ old('fecha_hasta', $filters['fecha_hasta'] ?? '') }}">
                    @error('fecha_hasta') <div class="form-error">{{ $message }}</div> @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="estado" name="estado">
                        ...
                        <option value="Diferida" {{ ($filters['estado'] ?? '') === 'Diferida' ? 'selected' : '' }}>Diferida</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select id="modalidad" name="modalidad">
                        ...
                        <option value="Presencial" {{ ($filters['modalidad'] ?? '') === 'Presencial' ? 'selected' : '' }}>Presencial</option>
                        <option value="Virtual" {{ ($filters['modalidad'] ?? '') === 'Virtual' ? 'selected' : '' }}>Virtual</option>
                        <option value="Híbrida" {{ ($filters['modalidad'] ?? '') === 'Híbrida' ? 'selected' : '' }}>Hibrida</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select id="tipo_audiencia_id" name="tipo_audiencia_id">
                        ...
                        @foreach($tipoAudiencias as $tipo)
                            <option value="{{ $tipo->id }}" {{ (string) ($filters['tipo_audiencia_id'] ?? '') === (string) $tipo->id ? 'selected' : '' }}>
                                {{ $tipo->tipo }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select id="juez_id" name="juez_id">
                        ...
                        @foreach($jueces as $juez)
                            <option value="{{ $juez->id }}" {{ (string) ($filters['juez_id'] ?? '') === (string) $juez->id ? 'selected' : '' }}>
                                {{ $juez->nombre }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="panel" style="padding: 24px;">
            <div style="display: flex; justify-content: space-between; align-items: center; gap: 16px; flex-wrap: wrap; margin-bottom: 18px;">
                ...
                <div class="actions">
                    <a href="{{ route('audiencias.report.export', request()->query()) }}" class="btn-info">Exportar Excel</a>
                    <a href="{{ route('audiencias.report.print', request()->query()) }}" class="btn-secondary" target="_blank">Imprimir / PDF</a>
                </div>
            </div>

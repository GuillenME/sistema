    ...
    <div class="form-card">
        ...
        <form action="{{ route('audiencias.update', $audiencia) }}" method="POST" class="form-grid">
            ...
            <div class="form-group full">
                ...
                <input type="text" id="causa" name="causa" value="{{ old('causa', $audiencia->causa) }}" required>
                @error('causa') <div class="form-error">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="tipo_audiencia_id" name="tipo_audiencia_id" required>
                    ...
                    @foreach($tipoAudiencias as $tipo)
                        <option value="{{ $tipo->id }}" {{ old('tipo_audiencia_id', $audiencia->tipo_audiencia_id) == $tipo->id ? 'selected' : '' }}>
                            {{ $tipo->tipo }}
                        </option>
                    @endforeach
                </select>
                @error('tipo_audiencia_id') <div class="form-error">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="delitos_id" name="delitos_id" required>
                    ...
                    @foreach($delitos as $delito)
                        <option value="{{ $delito->id }}" {{ old('delitos_id', $audiencia->delitos_id) == $delito->id ? 'selected' : '' }}>
                            {{ $delito->delito }}
                        </option>
                    @endforeach
                </select>
                @error('delitos_id') <div class="form-error">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="juez_id" name="juez_id" required>
                    ...
                    @foreach($jueces as $juez)
                        <option value="{{ $juez->id }}" {{ old('juez_id', $audiencia->juez_id) == $juez->id ? 'selected' : '' }}>
                            {{ $juez->nombre }}
                        </option>
                    @endforeach
                </select>
                @error('juez_id') <div class="form-error">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="salas_id" name="salas_id" required>
                    ...
                    @foreach($salas as $sala)
                        <option value="{{ $sala->id }}" {{ old('salas_id', $audiencia->salas_id) == $sala->id ? 'selected' : '' }}>
                            {{ $sala->sala }}
                        </option>
                    @endforeach
                </select>
                @error('salas_id') <div class="form-error">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="form-group full" data-imputados-picker>
                ...
                <div class="imputados-panel">
                    <div class="imputados-mode" role="group" aria-label="Cantidad de imputados">
                        <label>
                            <input type="radio" name="imputados_modo" value="single" {{ $imputadosMode === 'single' ? 'checked' : '' }}>
                            Un imputado
                        </label>
                        <label>
                            <input type="radio" name="imputados_modo" value="multiple" {{ $imputadosMode === 'multiple' ? 'checked' : '' }}>
                            Mas de uno
                        </label>
                    </div>

                    <input type="text" data-imputados-search placeholder="Buscar imputado por nombre o apellidos" autocomplete="off">

                    <div class="imputados-list">
                        @foreach($imputados as $imputado)
                            @php $fullName = trim($imputado->nombre . ' ' . $imputado->apellidos); @endphp
                            <label class="imputado-option" data-imputado-option data-search="{{ strtolower($fullName) }}">
                                <input
                                    type="checkbox"
                                    name="imputados[]"
                                    value="{{ $imputado->id }}"
                ...
                @error('imputados') <div class="form-error">{{ $message }}</div> @enderror
                @error('imputados.*') <div class="form-error">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="traductor_id" name="traductor_id">
                    ...
                    @foreach($traductores as $traductor)
                        <option value="{{ $traductor->id }}" {{ old('traductor_id', $audiencia->traductor_id) == $traductor->id ? 'selected' : '' }}>
                            {{ $traductor->nombres }}
                        </option>
                    @endforeach
                </select>
                @error('traductor_id') <div class="form-error">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="psicologo_id" name="psicologo_id">
                    ...
                    @foreach($psicologos as $psicologo)
                        <option value="{{ $psicologo->id }}" {{ old('psicologo_id', $audiencia->psicologo_id) == $psicologo->id ? 'selected' : '' }}>
                            {{ $psicologo->nombre }}
                        </option>
                    @endforeach
                </select>
                @error('psicologo_id') <div class="form-error">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="modalidad" name="modalidad" required>
                    <option value="">Selecciona</option>
                    <option value="Presencial" {{ old('modalidad', $audiencia->modalidad) == 'Presencial' ? 'selected' : '' }}>Presencial</option>
                    <option value="Virtual" {{ old('modalidad', $audiencia->modalidad) == 'Virtual' ? 'selected' : '' }}>Virtual</option>
                </select>
                @error('modalidad') <div class="form-error">{{ $message }}</div> @enderror
            </div>

            <div class="form-group">
                <label for="fecha">Fecha *</label>
                <input type="date" id="fecha" name="fecha" value="{{ old('fecha', optional($audiencia->fecha)->format('Y-m-d')) }}" required>
                @error('fecha') <div class="form-error">{{ $message }}</div> @enderror
            </div>

            <div class="form-group">
                <label for="hora">Hora *</label>
                <input type="time" id="hora" name="hora" value="{{ old('hora', optional($audiencia->hora)->format('H:i')) }}" required>
...
<script>
    document.querySelectorAll('[data-imputados-picker]').forEach(function (picker) {
        ...
        function updateHelp() {
            var selectedCount = checks.filter(function (input) { return input.checked; }).length;
            help.textContent = currentMode() === 'single'
                ? (selectedCount ? 'Hay 1 imputado seleccionado.' : 'Selecciona el imputado de la lista.')
                : (selectedCount ? 'Hay ' + selectedCount + ' imputados seleccionados.' : 'Selecciona todos los imputados que correspondan.');
        }

        modeInputs.forEach(function (input) {
            input.addEventListener('change', function () {
                if (input.value === 'single') {
                    var firstChecked = checks.find(function (check) { return check.checked; });
                    checks.forEach(function (check) { check.checked = check === firstChecked; });
                }
                updateHelp();
            });
        });
        ...
        checks.forEach(function (check) {
            check.addEventListener('change', function () {
                if (currentMode() === 'single' && check.checked) {
                    checks.forEach(function (other) { other.checked = other === check; });
                }
                updateHelp();
            });
        });
